(function(){
  ...
  function d3_stackedBars() {
    // Need to ensure that data is properly formatted: all bins must be present
    // in order to have proper ordering. Data is in the form:
    // BARS := SECTION*
    // SECTION := {...}
    // where sections have numeric values which can be stacked. Bars and
    // sections should also be identifiable (default, have a `name` attribute,
    // but this can be specified with barName and sectionName).
    //
    ...
    var margin = {top: 40, right: 200, bottom: 50, left: 40},
      ...
      sectionValue  = function(d) { return d.value; },
      sectionName = function(d) { return d.name; },
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        var barNames = data.map(function(d) {
          return barName(d);
        }),
        // TODO(ihodes) allow this to be specified...
